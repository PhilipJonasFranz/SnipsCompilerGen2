...
public class AsNAnd extends AsNNFoldExpression {
	...
	public static AsNAnd cast(And a, RegSet r, MemoryMap map, StackSet st) throws CGEN_EXC {
		AsNAnd and = new AsNAnd().pushCreatorStack(a);

		boolean isVFP = a.operands.stream().anyMatch(x -> x.getType().isFloat());
		...
		if (a.operands.get(0) instanceof Atom && a.operands.get(1) instanceof Atom) {
			...
			/* Load Operands */
			and.generatePrimitiveLoaderCode(and, a.operands.get(0), a.operands.get(1), r, map, st, 0, 1, isVFP);
